...
def store_array_in_a_file(size: int, preferred_file_name: str=None):
    global memory_error_reached, at_size
...
def measure_sorting_algorithm(algorithm_functions: list, from_n: int, to_n: int = None, range_given: bool = False, custom_file_name: str = None) -> list:
    ...
    for functions in algorithm_functions:
        ...
        total_algo_iterations = (to_value - from_n) * 10
        for i in range(from_n, to_value):
            if(stop_current_algo):
                break

            if(10**i != size_instance for size_instance in at_size):
                try_again = True
                while try_again:
                    try:
                        if(custom_file_name):
                ...
                for x in range(0, 10):
                    ...
                    if(two_minutes_reached and x >= 2):
                        #print(f"The algorithm got cut short: two minutes passed({two_minutes_reached})")
                        remaining_progress = total_algo_iterations - algorithm_progress
                        progress += remaining_progress
                        stop_current_algo = True
                        break
                    if(memory_error_reached):
                        print()
                        ...
                        #print(f"The algorithm got cut short: Memmory error reached({memory_error_reached})")
                        break
...
def display_algorithm_data_in_a_graph(algorithm_data: list):
    ...
    if(len(all_python_sort_results) > 0 or len(all_quick_sort_results) > 0 or len(all_self_made_results) > 0):
...
config = {
    "Sorting Functions: ": [self_made, python_sort],
    "From 10^: ": 2,
    "To 10^: ": 5,
    "Range: ": True,
